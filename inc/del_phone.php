...
if(isset($_GET['code1C'])){
    $c_id = getClientID($_GET['code1C']);
    if($c_id>0){
        $sql = "UPDATE phones SET del = 1 WHERE phone = '".$_GET['phone']."' AND client_id = ".$c_id;
        $dbc->element_free_update($sql);
        $out_row['result'] = 'OK';
    }
    else{
        $out_row['result'] = 'Клиент ненайден!';
    }
}
else{
    $out_row['result'] = 'Пустой код!';
}
