...
function getPostClientID($code_1C) {
	global $dbc;
	$row = $dbc->element_find_by_field('post_control','code_1C',$code_1C);
	return $row['id'];
}
...
// BSO полиса по id
function getPolisBsoFromId($polis_id) {
    global $dbc;
    $resp = $dbc->element_find('polises',$polis_id);
    return $resp['bso_number'];
}
...
// кода 1С оператора по d
function getUserIdFromCode1C($u_id) {
	global $dbc;
