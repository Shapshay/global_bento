...
function PolisCount() {
    ...
    $rows = $dbc->dbselect(array(
            ...
            "where"=>"(croots.office_id = 3) AND polises.dost <> ''  AND DATE_FORMAT(date_oform, '%Y%m%d') = '".date("Ymd")."'",
            "limit"=>"1"
        )
    );
    $numRows = $dbc->count;
...
$rows = $dbc->dbselect(array(
        ...
        "where"=>"DATE_FORMAT(date,'%Y-%m-%d')='".date("Y-m-d")."' AND (croots.office_id = 3)",
        "group"=>"croots.name"
    )
);
$all_count = 0;
...
$rows = $dbc->dbselect(array(
        ...
        "where"=>"(croots.office_id = 3) AND DATE_FORMAT(date,'%Y-%m-%d')='".date("Y-m-d")."'",
        "group"=>"croots.name"
    )
);
$all_pryan_count = 0;
...
$rows = $dbc->dbselect(array(
    ...
    "where"=>"(croots.office_id = 3) AND post_control.result <> 0
...
$_mailSubject = 'Отчеты Астана Bento';
$_mailFrom = "Bento CRM";
