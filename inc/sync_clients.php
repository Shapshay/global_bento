...
function lastClient(){
    global $dbc;
    $row = $dbc->element_find('sync_sv',1);
    if($dbc->count>0){
...
if($numRows>0){
    $filename = '/var/www/html/sync/clients_'.date("Ymd").'.txt';
    $fp = fopen ($filename, "w");
    foreach ($rows as $row){
        foreach ($row as $k=>$v){
            $row[$k] = trim(nl2probel($v));
        }
        fwrite($fp, serialize($row)."\r\n");
        $last_id = $row['id'];
    }
    fclose($fp);
    $dbc->element_update('sync_sv',1,array(
        "client_id" => $last_id));
}
