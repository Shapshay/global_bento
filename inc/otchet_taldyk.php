...
$rows = $dbc->dbselect(array(
        ...
        "where"=>"DATE_FORMAT(date,'%Y-%m-%d')='".date("Y-m-d")."' AND (croots.office_id = 4)",
        "group"=>"croots.name"
    )
);
$all_count = 0;
...
$rows = $dbc->dbselect(array(
        ...
        "where"=>"(croots.office_id = 4) AND DATE_FORMAT(date,'%Y-%m-%d')='".date("Y-m-d")."'",
        "group"=>"croots.name"
    )
);
$all_pryan_count = 0;
...
$rows = $dbc->dbselect(array(
    ...
    "where"=>"(croots.office_id = 4) AND post_control.result <> 0
...
$_mailSubject = 'Отчеты Талдыкурган Bento';
$_mailFrom = "Bento CRM";
...
sendMail3('aselshalahan@gmail.com', $_mailSubject, $mail_body, $_mailFrom, $_sendFrom);
sendMail3('www.madina.kz@inbox.ru', $_mailSubject, $mail_body, $_mailFrom, $_sendFrom);
