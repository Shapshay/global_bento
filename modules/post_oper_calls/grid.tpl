...
function PlayCall(AudioFile, oper_id, phone,res,res_id){
	...
	showControl();
	//alert(AudioFile);
    //console.log("http://192.168.4.200/freeswitch/"+AudioFile);
	audioPlayer.attr({
          src: "http://192.168.4.200/freeswitch/"+AudioFile,
          autoplay: "autoplay"
		});
...
function saveControl(){
	...
	$.post("modules/post_oper_calls/control.php", {oper_id: oper_id, ROOT_ID: ROOT_ID, phone: phone, Ocenka: Ocenka, res_id:res_id},
		function(data){
			//alert(data);
			closeControl();
		});
...
function ShowStatTable(){
	...
	$('#table_rows').html('');
	$('#waitGear').show();
	$.post("modules/post_oper_calls/show_stat.php", {oper_id:oper_id, stat_id:stat_id, date_start:date_start, date_end:date_end, limit:limit},
			function(data){
				//alert(data);
