...
if(!$search){
	...
	foreach($array['return'] as $clients){
		if(isset($clients[0]['Code1C'])){
			foreach($clients as $client){
				...
				$tpl->assign("RESULT_CODE1C", $client['Code1C']);
            ...
            $tpl->assign("RESULT_CODE1C", $clients['Code1C']);
    ...
    foreach($rows as $row){
        ...
        if(in_array(1,$this_role)||in_array(2,$this_role)){
            $oper_rows.='<option value="'.$row['login_1C'].'">'.$row['name'];
        }
    }
    $tpl->assign("PEREBROS_OPERS", $oper_rows);
