...
$j=0;
foreach($rows as $row){
    $row2 = $dbc->element_find_by_field('sto','code_1C',$row['code_1C']);
    if($dbc->count==0){
        echo "<p>+".$dbc->outsql;
        //print_r($row);
        $dbc->element_create('sto',array(
            "sto_tochka_id" => $row['sto_tochka_id'],
            "code_1C" => $row['code_1C'],
            "oper_id" => $row['oper_id'],
            "name" => $row['name'],
            "iin" => $row['iin'],
            ...
            "res_call_id" => $row['res_call_id'],
            "date_dog" => $row['date_dog'],
            "err_res_id" => $row['err_res_id'],
            "visit" => $row['visit'],
            "date_visit" => $row['date_visit'],
            "status_to" => $row['status_to'],
            "summa" => $row['summa'],
            "strach" => $row['strach'],
            "strach_company" => $row['strach_company'],
            "date_strach_start" => $row['date_strach_start'],
            "date_strach_end" => $row['date_strach_end']
        ));
        $i++;
    }
    else{
